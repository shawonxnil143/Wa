// IrfanBot – Render Complete (CommonJS, Node 18)
...
const { backupAuthDir, restoreAuthDir } = require('./utils/sessionStore');
...
async function start(){
  // DB connect (optional)
  const dbState = await connectDB(logger);

  // If sessionBackup enabled and db connected, restore auth/ before login
  const authDir = path.join(__dirname,'auth');
  if (CONFIG.database?.sessionBackup && dbState.connected) {
    await restoreAuthDir(authDir, logger);
  }
  ...
  // backup session to DB whenever creds update
  sock.ev.on('creds.update', async () => {
    await saveCreds();
    if (CONFIG.database?.sessionBackup && dbState.connected) {
      await backupAuthDir(authDir, logger);
    }
  });
