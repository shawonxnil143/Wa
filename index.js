...
// Volatile (in-memory) commands for testing: lost on restart
...
function unregisterCommandByName(name, commands) {
  volatileCommands.delete(name); // also drop volatile if exists
  if (commands.has(name)) { commands.delete(name); return true; }
  return false;
}
...
// NEW: register from code (non-persistent; for testing only) + sandboxed require('axios')
function registerCommandFromCode(name, code, logger) {
  if (!/^[a-z0-9_\-]{2,32}$/i.test(name)) throw new Error('Invalid name (2–32 chars a-z0-9_-)');

  // Allowlist for require()
  const ALLOWED = new Set(['axios']);
  const safeRequire = (mod) => {
    if (!ALLOWED.has(mod)) throw new Error(`Blocked require("${mod}")`);
    return require(mod);
  };

  // Wrap raw body if user didn't provide module.exports
  let moduleCode = code;
...
module.exports = {
  ...
  try {
    // Expose limited require to user code
    // eslint-disable-next-line no-new-func
    const factory = new Function('module', 'exports', 'require', moduleCode);
    const mod = { exports: {} };
    factory(mod, mod.exports, safeRequire);
    ...
    volatileCommands.set(name, mod.exports);
    logger?.info?.(`🧪 Volatile command registered: ${name} (will be lost on restart)`);
    return mod.exports;
  } catch (e) {
...
async function start() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    m.reply = async (message, options = {}) =>
      sock.sendMessage(jid, { text: String(message), ...options }, { quoted: m });
    ...
    // Resolve command: volatile > loaded files
    const cmd = volatileCommands.get(name?.toLowerCase()) || commands.get(name?.toLowerCase());
    if (!cmd) return;
      ...
      await cmd.run({
        ...
        helpers: {
          ...
          registerCommandFromCode: (nm, code) => {
            ...
            commands.set(mod.name, mod); // expose via main map too
            return mod;
          },
          customDir,
          volatileCommands
