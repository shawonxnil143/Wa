...
// Keep last pairing code in memory (for /pairing-code)
...
// 🔐 fetch pairing code via HTTP (requires X-Admin-Key)
app.get('/pairing-code', (req, res) => {
  if (!checkAdminKey(req)) return res.status(401).json({ ok: false, error: 'Invalid admin key' });
  if (!LAST_PAIR_CODE) return res.status(404).json({ ok: false, error: 'No pairing code generated yet' });
  res.json({ ok: true, phone: (CONFIG.botNumber || '').replace(/[^0-9]/g, ''), code: LAST_PAIR_CODE });
});
...
// use manager to load and manage commands
...
async function start() {
  ...
  async function maybeShowPairCode() {
    ...
    try {
      ...
      // ✅ One-line, clean log — easy to copy in Render
      console.log(`PAIRING_CODE=${code} PHONE=${phone} (WhatsApp > Linked devices > Link with phone)`);
    } catch (e) {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      await cmd.run({
        ...
        commands,
        // manager helpers (install/reload etc.)
        helpers: mgr.createHelpers(commands, logger, customDir)
      });
    } catch (e) {
      logger.error(e);
